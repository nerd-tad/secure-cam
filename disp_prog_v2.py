...
c = 0   #flag used to identify problems with monitoring device (can accumilate incrementally)
d = 0   #flag to identify problems with secure cam.  only 0(for connection ok) and 1(for connection error)
...
while 1:
    ...
    if data == 404: #and c == 0, like that you could simplify wisely the code.....
        ...
        if dell > 50 and d == 0:   #d is to identify the number of times that was identified , there is a problem in connection on secure cam....
        ...
        if (detected_obj != 'static') and dell <= 25:#if (detected_obj != 'nothing' or detected_obj != 'static') and dell <= 25:
            if d > 0:
                ...
                engine_sound.runAndWait()
            if same_object_counter == 0:
                marker = detected_obj
            if marker == detected_obj:
                same_object_counter = same_object_counter+1
                print('same_object_counter', same_object_counter)
            if same_object_counter < 2:
                d = 0
